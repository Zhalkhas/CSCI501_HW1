...
PPMImage readPPM ( char * fileName )
{
  ...
  while ( fgets( line , 1024 , file ) )
  {
    ...
    if ( commentLocation != NULL )  { commentLocation[0] = '\n' ;  commentLocation[1] = '\0' ; }
    ...
    while ( ! lineConsumed )
    {
      switch ( state )
      {
        ...
        case SMAGICNUMBER   : if ( sscanf( cursor , " P3%n" , &numberOfCharactersRead ) >= 0 )
                              {
                                state   = SWIDTH                 ;
        ...
        case SWIDTH         : if ( sscanf( cursor , "%hi%n" , &(image.width) , &numberOfCharactersRead ) > 0 )
                              {
                                state   = SHEIGHT                ;
                                cursor += numberOfCharactersRead ;
                              }
        ...
        case SHEIGHT        : if ( sscanf( cursor , "%hi%n" , &(image.height) , &numberOfCharactersRead ) > 0 )
                              {
                                state   = SMAXCOLORVALUE         ;
                                cursor += numberOfCharactersRead ;

                                image.pixels = (PPMPixel *) malloc( image.width * image.height * sizeof( PPMPixel ) ) ;
                              }
                              else
                              {
                                lineConsumed = 1 ;
        ...
        case SMAXCOLORVALUE : if ( sscanf( cursor , "%hi%n" , &(image.maxColorValue) , &numberOfCharactersRead ) > 0 )
                              {
                                state   = SRED                   ;
                                cursor += numberOfCharactersRead ;
                              }
        ...
        case SRED           : if ( sscanf( cursor , "%hi%n" , &(image.pixels[currentPixelIndex].r) , &numberOfCharactersRead ) > 0 )
                              {
                                state   = SGREEN                 ;
                                cursor += numberOfCharactersRead ;
                              }
        ...
        case SGREEN         : if ( sscanf( cursor , "%hi%n" , &(image.pixels[currentPixelIndex].g) , &numberOfCharactersRead ) > 0 )
                              {
                                state   = SBLUE                  ;
                                cursor += numberOfCharactersRead ;
                              }
        ...
        case SBLUE          : if ( sscanf( cursor , "%hi%n" , &(image.pixels[currentPixelIndex].b) , &numberOfCharactersRead ) > 0 )
                              {
                                state   = SRED                   ;
                                cursor += numberOfCharactersRead ;
