...
bool RealIsWithinBounds ( double value , double lowerLimit , double upperLimit )
{
  ...
  if   ( value >= (lowerLimit - tolerance) && value <= (upperLimit + tolerance) )  { return true  ; }
  else                                                                             { return false ; }
}
...
{
  if   ( r < g )  { if ( r < b )  { return r ; }  else { return b ; } }
  else            { if ( g < b )  { return g ; }  else { return b ; } }
...
bool RgbF_IsValid ( double r , double g , double b )
{
  ...
  if ( ( RealIsWithinBounds( r , PER_LOWER_LIMIT , PER_UPPER_LIMIT ) == false ) ||
       ...
       ( RealIsWithinBounds( b , PER_LOWER_LIMIT , PER_UPPER_LIMIT ) == false )  )  { isValid = false ; }
...
bool RgbI_IsValid ( uint8_t r , uint8_t g , uint8_t b )
{
  ...
  if ( ( IntegerIsWithinBounds( r , RGBI_LOWER_LIMIT , RGBI_UPPER_LIMIT ) == false ) ||
       ...
       ( IntegerIsWithinBounds( b , RGBI_LOWER_LIMIT , RGBI_UPPER_LIMIT ) == false )  )  { isValid = false ; }
...
bool Hsv_IsValid ( double h , double s , double v )
{
  ...
  if ( ( RealIsWithinBounds( h , HUE_LOWER_LIMIT , HUE_UPPER_LIMIT ) == false ) ||
       ...
       ( RealIsWithinBounds( v , PER_LOWER_LIMIT , PER_UPPER_LIMIT ) == false )  )  { isValid = false ; }
...
bool Hsi_IsValid ( double h , double s , double i )
{
  ...
  if ( ( RealIsWithinBounds( h , HUE_LOWER_LIMIT , HUE_UPPER_LIMIT ) == false ) ||
       ( RealIsWithinBounds( s , PER_LOWER_LIMIT , PER_UPPER_LIMIT ) == false ) ||
       ( RealIsWithinBounds( i , PER_LOWER_LIMIT , PER_UPPER_LIMIT ) == false )  )  { isValid = false ; }
...
bool Hsl_IsValid ( double h , double s , double l )
{
  ...
  if ( ( RealIsWithinBounds( h , HUE_LOWER_LIMIT , HUE_UPPER_LIMIT ) == false ) ||
       ( RealIsWithinBounds( s , PER_LOWER_LIMIT , PER_UPPER_LIMIT ) == false ) ||
       ( RealIsWithinBounds( l , PER_LOWER_LIMIT , PER_UPPER_LIMIT ) == false )  )  { isValid = false ; }
...
bool Yiq_IsValid ( double y , double i , double q )
{
  ...
  if ( ( RealIsWithinBounds( y ,   PER_LOWER_LIMIT ,   PER_UPPER_LIMIT ) == false ) ||
       ...
       ( RealIsWithinBounds( q , YIQ_Q_LOWER_LIMIT , YIQ_Q_UPPER_LIMIT ) == false )  )  { isValid = false ; }
...
bool Yuv_IsValid ( double y , double u , double v )
{
  ...
  if ( ( RealIsWithinBounds( y ,   PER_LOWER_LIMIT ,   PER_UPPER_LIMIT ) == false ) ||
       ...
       ( RealIsWithinBounds( v , YUV_V_LOWER_LIMIT , YUV_V_UPPER_LIMIT ) == false )  )  { isValid = false ; }
...
RgbFColor * RgbF_CreateFromHsv ( double h , double s , double v )
{
  ...
  if ( Hsv_IsValid( h , s , v ) == true )
  {
    ...
    if      ( h >=   0.0 && h <  60.0 )  { color = RgbF_Create( c + m , x + m ,     m ) ; }
    else if ( h >=  60.0 && h < 120.0 )  { color = RgbF_Create( x + m , c + m ,     m ) ; }
    else if ( h >= 120.0 && h < 180.0 )  { color = RgbF_Create(     m , c + m , x + m ) ; }
    else if ( h >= 180.0 && h < 240.0 )  { color = RgbF_Create(     m , x + m , c + m ) ; }
    else if ( h >= 240.0 && h < 300.0 )  { color = RgbF_Create( x + m ,     m , c + m ) ; }
    else if ( h >= 300.0 && h < 360.0 )  { color = RgbF_Create( c + m ,     m , x + m ) ; }
    else                                 { color = RgbF_Create(     m ,     m ,     m ) ; }
  }
...
RgbFColor * RgbF_CreateFromHsl ( double h , double s , double l )
{
  ...
  if ( Hsl_IsValid( h , s , l ) == true )
  {
    ...
    if      ( h >= (                   0.0       ) && h < (       HUE_UPPER_LIMIT / 6.0 ) )  { color = RgbF_Create( c + m , x + m ,     m ) ; }
    else if ( h >= (       HUE_UPPER_LIMIT / 6.0 ) && h < (       HUE_UPPER_LIMIT / 3.0 ) )  { color = RgbF_Create( x + m , c + m ,     m ) ; }
    else if ( h >= (       HUE_UPPER_LIMIT / 3.0 ) && h < (       HUE_UPPER_LIMIT / 2.0 ) )  { color = RgbF_Create(     m , c + m , x + m ) ; }
    else if ( h >= (       HUE_UPPER_LIMIT / 2.0 ) && h < ( 2.0 * HUE_UPPER_LIMIT / 3.0 ) )  { color = RgbF_Create(     m , x + m , c + m ) ; }
    else if ( h >= ( 2.0 * HUE_UPPER_LIMIT / 3.0 ) && h < ( 5.0 * HUE_UPPER_LIMIT / 6.0 ) )  { color = RgbF_Create( x + m ,     m , c + m ) ; }
    else if ( h >= ( 5.0 * HUE_UPPER_LIMIT / 6.0 ) && h < (       HUE_UPPER_LIMIT       ) )  { color = RgbF_Create( c + m ,     m , x + m ) ; }
    else                                                                                     { color = RgbF_Create(     m ,     m ,     m ) ; }
  }
...
HsvColor * Hsv_CreateFromRgbF ( double r , double g , double b )
{
  ...
  if ( RgbF_IsValid( r , g , b ) == true )
  {
    ...
    {
      /*
      if      ( M == r )   { color->H = fmod( ( ( g - b ) / c ) , 6.0 ) ; }
      else if ( M == g )   { color->H = ( b - r ) / c + 2.0             ; }
      else  **( M == b )** { color->H = ( r - g ) / c + 4.0             ; }

      color->H *= 60.0         ;
      */

      if      ( M == r )   { color->H = fmod( (60 * ((g - b) / c) + 360) , 360.0 ) ; }
      else if ( M == g )   { color->H = fmod( (60 * ((b - r) / c) + 120) , 360.0 ) ; }
      else  /*( M == b )*/ { color->H = fmod( (60 * ((r - g) / c) + 240) , 360.0 ) ; }
